...
#
# The browser uses localhost, the origin next dev serves its dev assets and HMR to.
set -euo pipefail
...
wait_for() {
  ...
  until eval "$check"; do
    ...
    if [ "$waited" -ge "$READY_TIMEOUT_SECS" ]; then
      echo "$what was not up after ${READY_TIMEOUT_SECS}s" >&2
      exit 1
    fi
    sleep 3
    waited=$((waited + 3))
...
wait_for "demo:record" 'grep -q "Demo is up" "$LOG"'

wait_for "web" 'curl -fsS -o /dev/null "http://localhost:${WEB_PORT}/manifest.webmanifest"'
