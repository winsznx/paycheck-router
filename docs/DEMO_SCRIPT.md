...
| 2:35–2:55 | Proof | `/proof` page, then a terminal running `pnpm verify:bundle evidence/stocklana-fork` printing PASS per slice, then `pnpm demo:fork` starting | "Every slice has a public proof, and anyone can re-check it against Pyth history with one command, or replay the whole run from a clean clone." |
...
| Time | Beat | On screen |
| --- | --- | --- |
| 0:00–0:40 | Accounts and PDAs | Repository `programs/paycheck_router/src/state.rs` and `instructions/execute.rs`: Config, Asset, Router, Authority, Convert authority, Paycheck, with their seeds. The Authority PDA is the SPL delegate on the owner's USDC and the Jupiter taker, and holds nothing between instructions. |
